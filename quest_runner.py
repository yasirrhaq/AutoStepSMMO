...
def input_with_timeout(
    prompt: str = "", timeout: float = 10.0, default: str = ""
) -> str:
    """
    Get user input with a timeout. If no input is received within timeout, returns default.

    Args:
        prompt: The prompt to display to the user
        timeout: Timeout in seconds (default: 10)
        default: Default value if timeout occurs (default: empty string)
    ...
    if sys.platform.startswith("win"):
        ...
        # Unix-like systems (Linux/Mac) - fallback without timeout
        try:
            return input(prompt)
        except EOFError:
            return default
...
class QuestRunner(SimpleMMOBot):
    ...
    def __init__(self, config_file: str = "config.json"):
        ...
        logging.basicConfig(
            ...
            handlers=[logging.FileHandler("quest_runner.log"), logging.StreamHandler()],
        )
    ...
    def extract_quest_point_info(self, page_html: str) -> Dict[str, Any]:
        ...
        try:
            ...
            max_m = re.search(
                r'"expedition\.user\.max_quest_points"\s*:\s*(\d+)', page_html
            )
            if qp_m:
    ...
    def wait_for_quest_points(self):
        ...
        while True:
            ...
            self.logger.info(
                f"Quest Points: {qp_str}{max_str} — checking again in {POLL_SECS}s"
            )
            ...
            for remaining in range(POLL_SECS, 0, -10):
                print(
                    f"\r  [QP wait] Rechecking in {remaining:3d}s ...  ",
                    end="",
                    flush=True,
                )
    ...
    def extract_quest_api_endpoints(self, page_html: str) -> Dict[str, str]:
        ...
        try:
            ...
            if perform_match:
                ...
                self.logger.debug(
                    f"Found perform endpoint: {endpoints['perform_url'][:60]}..."
                )
    ...
    def get_available_quests(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(
                    f"Failed to fetch quests page: HTTP {response.status_code}"
                )
                return []
            ...
            if api_response.status_code != 200:
                self.logger.error(
                    f"Failed to fetch quest API: HTTP {api_response.status_code}"
                )
                self.logger.error(f"Response: {api_response.text[:500]}")
                # Check if signature expired
                if (
                    "expired" in api_response.text.lower()
                    or "signature" in api_response.text.lower()
                ):
                    self.logger.error(
                        "Signed URL may have expired. Try refreshing the quest page."
                    )
                return []
            ...
            if expeditions:
                ...
                self.logger.debug(
                    f"First expedition raw keys: {list(first_exp.keys())}"
                )
                level_candidates = {
                    k: v for k, v in first_exp.items() if "level" in k.lower()
                }
                if level_candidates:
                    self.logger.info(f"Level-related fields in API: {level_candidates}")
                else:
                    self.logger.warning(
                        "No 'level' fields found in expedition data — level sorting will use ID as tiebreaker"
                    )
            ...
            for exp in expeditions:
                quest_data = {
                    ...
                    "remaining": exp.get("amount_to_complete", 0)
                    - exp.get("amount_completed", exp.get("completed_amount", 0)),
                    "experience": exp.get("experience", 0),
                    "gold": exp.get("gold", 0),
                    "success_chance": int(
                        exp.get(
                            "success_chance",
                            exp.get("chance", exp.get("success_rate", 100)),
                        )
                        or 100
                    ),
                ...
                if quest_data["id"] and quest_data["title"]:
                    ...
                    self.logger.debug(
                        f"Parsed quest: {quest_data['title']} (ID: {quest_data['id']}, Level: {quest_data['level_required']}, Completed: {quest_data['is_completed']})"
                    )
    ...
    def perform_quest(self, quest_data: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Ensure exp and gold are integers (may be strings from API with commas)
                try:
                    exp = int(str(exp).replace(",", ""))
                except (ValueError, TypeError):
                    exp = 0
                try:
                    gold = int(str(gold).replace(",", ""))
                except (ValueError, TypeError):
                    gold = 0
    ...
    def auto_quest_loop(
        ...
        for _pq_num, (_pquest, _ptimes) in enumerate(priority_quests or [], start=1):
            ...
            while _p_done < _ptimes:
                ...
                if result.get("success"):
                    ...
                    quest_steps_done += 1
                    quest_progress[_ptitle]["done"] = (
                        quest_progress[_ptitle].get("done", 0) + 1
                    )
                    ...
                    self._session_quest_progress = dict(quest_progress)
                    self.logger.info(
                        f"Priority quest {_pq_num}/{_total_pq} progress: {_p_done}/{_ptimes}"
                    )
                    if _p_done < _ptimes:
                        delay = self._get_delay(
                            self.quest_delay_steps_min, self.quest_delay_steps_max
                        )
                    ...
                    if result.get("should_stop"):
                        self.logger.warning(
                            "Out of quest points — waiting for regen..."
                        )
            ...
            if _pq_num < _total_pq:
                delay = self._get_delay(
                    self.quest_delay_quests_min, self.quest_delay_quests_max
                )
        ...
        while True:
            ...
            self.logger.info(
                f"Level Requirement: {current_quest.get('level_required', '?')}"
            )
            ...
            while quest_remaining > 0:
                ...
                if result.get("success"):
                    ...
                    if result.get("should_stop"):
                        self.logger.warning(
                            "Out of quest points — pausing until regen..."
                        )
                        self.wait_for_quest_points()
                        ...
                        self.logger.error(
                            "Session expired - please restart with fresh tokens"
                        )
                        return
                    elif "rate limit" in error.lower() or error == "Cooldown":
                        wait = result.get("retry_after", 30)
                        self.logger.warning(
                            f"Rate limited / cooldown - waiting {wait} seconds..."
                        )
                        time.sleep(wait)
                    else:
                        self.logger.warning(
                            f"Waiting {self.quest_error_retry_delay}s before retry..."
                        )
                        time.sleep(self.quest_error_retry_delay)
        ...
        if quest_progress:
            ...
            for _title, _info in quest_progress.items():
                ...
                _status = (
                    ...
                    else f"{_info['done']}/{_info['total']} done, {_info['remaining']} left"
                )
...
def main():
    ...
    if incomplete:
        ...
        for i, q in enumerate(incomplete, 1):
            ...
            sc_label = (
                f"  \033[91m[{sc_tag}]\033[0m"
                if sc < 100
                else f"  \033[92m[{sc_tag}]\033[0m"
            )
            print(
                f"  {i:>3}. [Lvl {lvl:>6}] {q['title']}  (remaining: {rem}){sc_label}"
            )
        print("=" * 60)
        print()
        ...
        while True:
            choice = input_with_timeout(
                f"  Quest #{slot} (10s = auto start, Enter to start bot): ",
                timeout=10,
                default="",
            )
            choice = choice.strip()
            ...
            try:
                ...
                if 0 <= idx < len(incomplete):
                    ...
                    times = (
                        int(times_str)
                        if times_str.isdigit() and int(times_str) > 0
                        else 1
                    )
        ...
        print()
        dir_choice = input_with_timeout(
            "  Direction (L/H, 10s no response = H): ", timeout=10, default="h"
        )
        direction = "highest" if dir_choice.strip().lower() == "l" else "highest"
        if direction == "highest":
            print("  → Highest-level 100% quests first, working downward.\n")
        else:
            print("  → Lowest-level quests first.\n")
    else:
    ...
    try:
        signal.signal(
            signal.SIGBREAK, _handle_shutdown
        )  # Windows: Ctrl+Close / window X
            ...
            if _qp:
                ...
                for _title, _info in _qp.items():
                    ...
                    _status = (
                        "completed"
                        if _remaining == 0
                        else f"{_done}/{_total} done, {_remaining} left"
                    )
                    print(f"   [{_icon}] {_title}: {_status}")
            print("=" * 60)

            # Ask user to restart or exit (10s timeout = auto restart)
            restart = input_with_timeout(
                "\n Press Enter to restart, or C to close (10s until auto restart): ",
                timeout=10,
                default="",
            )
            if restart.strip().lower() == "c":
